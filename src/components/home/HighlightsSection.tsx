...
import { Home, Compass, Waves, TreePine, Utensils, Shield, Heart } from "lucide-react";
const highlights = [{
  icon: Home,
  title: "Luxury Villas",
  ...
  icon: Waves,
  title: "Infinity Pool",
  description: "Stunning infinity pool overlooking the forest canopy"
}, {
  icon: TreePine,
  title: "Nature Walks",
...
export function HighlightsSection() {
  ...
  return <section ref={ref} className="py-24 md:py-32 lg:py-40 bg-cream relative overflow-hidden">
      ...
      <div className="luxury-container relative">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 lg:gap-4">
          {highlights.map(item => <motion.div key={item.title} variants={itemVariants} className="group text-center p-6 lg:p-4 rounded-2xl hover:bg-background transition-all duration-500">

  ...
  return useMutation({
    mutationFn: async ({ formData, priceBreakdown }: CreateBookingParams): Promise<Booking> => {
      ...
      return {
        ...
        meal_plan: data.meal_plan as MealPlan,
        room_total: Number(data.room_total),
        extra_guest_total: Number(data.extra_guest_total),
        meal_plan_total: Number(data.meal_plan_total),
        package_total: Number(data.package_total),
